...
function parseModuleFile(filePath) {
  ...
  const sourceFileMatch = src.match(/source: WEB7 Symbiosis WebAPI (\S+)\)/);
...
const { isError, message, result } = await web7.${clientProp}.${exampleOp.jsName}(${exampleArgs});
...
const indexRows = moduleSummaries
  .map((m) => `| [\`web7.${m.clientProp}\`](modules/${m.moduleName}.md) | \`${m.routePrefix}\` | ${m.opCount} |`)
  .join('\n');

const totalOps = moduleSummaries.reduce((n, m) => n + m.opCount, 0);

const indexContent = `# WEB7 Symbiosis & Collective Consciousness API — JavaScript SDK Reference
